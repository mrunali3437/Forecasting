...
abline(reg~lm(new_data~time(new_data)))
...
new_data <- cbind(airlines, x) #cbind function is used to combine the columns of the datasets
View(new_data)
...
# By principal of parsimony we will consider lag - 6  as we have so many significant lags 
...
#Seeing the plot, we get to know that the forecast done was not accurate. This happens when we don't provide the right
# p-value and q-value to the model.

#If we don't know the p-vale and q-value for the model then we can build the model using auto.arima() function.
#This function will analyze the p and q value and build a proper model. Lets Build the Model

ab <- auto.arima(airline_ts, trace = T)
summary(ab)
attributes(ab)
windows()
